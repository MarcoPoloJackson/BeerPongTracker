...
class Player(db.Model):
    __tablename__ = 'player'  # Manteniamo il nome 'player'
    id = db.Column(db.Integer, primary_key=True)
    name = db.Column(db.String(50), unique=True, nullable=False)  # Il nome usato in gioco

    # NUOVE COLONNE AGGIUNTE PER L'AUTENTICAZIONE
    password = db.Column(db.String(50), nullable=False)  # Sarà obbligatoria, ma nullable temporaneamente per l'aggiunta
...
def get_player_db_session(player_name):
    ...
    if not os.path.exists(db_filename):
        try:
            conn = sqlite3.connect(db_filename); conn.close()
        except:
            pass
...
def select_player():
    with app.app_context():
        # --- PULIZIA: Rimuovi tutti i blocchi 'try: with db.engine.connect()...' ---
        # Dopo aver eseguito una volta db.create_all(), queste migrazioni
        # (per colonne come redemption_hits, t1_format_changed, t1_pending_list)
        # dovrebbero essere rimosse da qui, per evitare errori.

        pass  # Mantieni questo 'pass' o rimuovi il blocco 'with app.app_context()' se vuoto.

    # 1. Carica TUTTI i giocatori dalla tabella 'player' (che ora ha anche password e edit)
    players = Player.query.order_by(Player.name).all()

    # 2. Carica le partite attive (logica invariata)
    ...
    for m in active_matches:
        if m.status != 'finished':
            # Se hai deciso di non salvare i nomi dei giocatori nel modello ActiveMatch,
            # dovresti aggiornare ActiveMatch, ma per ora il codice è corretto
            # assumendo che m.t1_p1, etc. contengano ancora i nomi dei giocatori.
    ...
    return render_template('select_player.html',
                           players=players,
                           # La lista 'players' ora contiene oggetti Player con 'name', 'password', 'edit'
...
def index(player_name):
    ...
    if match:
        ...
        try:
            ...
            if team == 't1':
                pending_damage_for_me = match.pending_damage_for_t1; pending_damage_for_them = match.pending_damage_for_t2
            else:
                pending_damage_for_me = match.pending_damage_for_t2; pending_damage_for_them = match.pending_damage_for_t1
...
def add_player():
    ...
    password_input = request.form['password']  # Recuperiamo la password inviata dal form

    # 1. GENERAZIONE DELL'HASH DELLA PASSWORD
    hashed_password = generate_password_hash(password_input)

    # 2. Controllo e Inserimento nel DB
    if pn and not Player.query.filter_by(name=pn).first():
        new_player = Player(
            name=pn,
            password=hashed_password,  # Salviamo l'hash, non la password in chiaro
            edit=False  # Impostato a FALSE di default, come richiesto
        )
